        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('vendors.store') }}" class="space-y-6 max-w-2xl">
                        @csrf
                        ...
                        <div>
                            ...
                            <select id="status" name="status" class="mt-1 block w-full border-gray-300 focus:border-primary-500 focus:ring-primary-500 rounded-md shadow-sm">
                                ...
                                <option value="Pernah Transaksi">Pernah Transaksi</option>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-prsystem::text-input id="category" class="block mt-1 w-full" type="text" name="category" :value="old('category')" />
                                <x-prsystem::input-error :messages="$errors->get('category')" class="mt-2" />
                            </div>
                            <div>
                                <x-prsystem::input-label for="location" :value="__('Lokasi')" />
                                <x-prsystem::text-input id="location" class="block mt-1 w-full" type="text" name="location" :value="old('location')" />
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3" class="block mt-1 w-full border-gray-300 focus:border-primary-500 focus:ring-primary-500 rounded-md shadow-sm">{{ old('description') }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <x-prsystem::text-input id="pic_name" class="block mt-1 w-full" type="text" name="pic_name" :value="old('pic_name')" />
                            </div>
                            <div>
                                <x-prsystem::input-label for="phone" :value="__('No Telpon / HP')" />
                                <x-prsystem::text-input id="phone" class="block mt-1 w-full" type="text" name="phone" :value="old('phone')" />
                            </div>
                            <div>
                                <x-prsystem::input-label for="email" :value="__('Email')" />
                                <x-prsystem::text-input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email')" />
                            </div>
                        </div>

                        <div class="flex items-center gap-4">
                            <x-prsystem::primary-button>{{ __('Save Supplier') }}</x-prsystem::primary-button>
