        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('sub-departments.update', $subDepartment) }}" class="space-y-6 max-w-xl">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <x-prsystem::text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $subDepartment->name)" required autofocus />
                            <x-prsystem::input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        <div>
                            <x-prsystem::input-label for="coa" :value="__('COA (Optional)')" />
                            <x-prsystem::text-input id="coa" class="block mt-1 w-full" type="text" name="coa" :value="old('coa', $subDepartment->coa)" />
                            <x-prsystem::input-error :messages="$errors->get('coa')" class="mt-2" />
                        </div>

                        <div class="flex items-center gap-4">
                            <x-prsystem::primary-button>{{ __('Update Sub Department') }}</x-prsystem::primary-button>
