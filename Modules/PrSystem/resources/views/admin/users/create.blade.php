        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <form action="{{ route('users.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <x-prsystem::text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')" required autofocus />
                    <x-prsystem::input-error class="mt-2" :messages="$errors->get('name')" />
                </div>

                <div>
                    <x-prsystem::input-label for="email" :value="__('Email')" />
                    <x-prsystem::text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email')" required />
                ...
                <div>
                     ...
                     <x-prsystem::text-input id="phone_number" name="phone_number" type="text" class="mt-1 block w-full" :value="old('phone_number')" />
                ...
                <div>
                    ...
                    <select id="global_role" name="global_role" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                        ...
                        @foreach($roles as $role)
                            <option value="{{ $role->name }}" {{ old('global_role', old('role')) == $role->name ? 'selected' : '' }}>{{ ucfirst($role->name) }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-prsystem::text-input id="position" name="position" type="text" class="mt-1 block w-full" :value="old('position')" />
                ...
                <div>
                    ...
                    <x-prsystem::text-input id="password" name="password" type="password" class="mt-1 block w-full" required autocomplete="new-password" />
                    <x-prsystem::input-error class="mt-2" :messages="$errors->get('password')" />
                </div>

                <!-- Confirm Password -->
                <div>
                    <x-prsystem::input-label for="password_confirmation" :value="__('Konfirmasi Password')" />
                    <x-prsystem::text-input id="password_confirmation" name="password_confirmation" type="password" class="mt-1 block w-full" required />
                ...
                <div class="flex justify-end gap-4">
                    ...
                    <x-prsystem::primary-button>{{ __('Simpan') }}</x-prsystem::primary-button>
                </div>
            </form>
        </div>
    </div>
