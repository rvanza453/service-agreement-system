<x-serviceagreementsystem::layouts.master :title="'Edit Skema Approval'">
    <div class="d-flex justify-between align-center mb-4">
        <div>
            <h1 style="font-size: 24px; font-weight: 700; color: var(--text-primary);">Edit Skema: {{ $approval_schema->name }}</h1>
            <p class="text-muted" style="font-size: 14px;">Modifikasi alur persetujuan dan departemen terkait.</p>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('sas.approval-schemas.index') }}" class="btn btn-secondary">Batal</a>
            <button type="submit" form="schemaForm" class="btn btn-primary">Simpan Perubahan</button>
        </div>
    </div>

    <form id="schemaForm" action="{{ route('sas.approval-schemas.update', $approval_schema) }}" method="POST">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="name" id="name" class="form-control" required value="{{ old('name', $approval_schema->name) }}">
                    </div>
                    <div class="form-group" style="display: flex; align-items: flex-end; padding-bottom: 4px;">
                        <label class="d-flex align-center gap-2" style="cursor: pointer;">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $approval_schema->is_active) ? 'checked' : '' }} style="width: 18px; height: 18px; cursor: pointer;">
                ...
                <div class="form-group mb-0">
                    ...
                    <textarea id="description" name="description" class="form-control" rows="3">{{ old('description', $approval_schema->description) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 16px;">
                    @php
                        $selectedDepts = old('departments', $approval_schema->departments->pluck('id')->toArray());
                    @endphp
                    ...
                    <label class="d-flex align-center gap-2" style="padding: 10px; border: 1px solid var(--border-color); border-radius: 8px; cursor: pointer; transition: var(--transition);">
                        <input type="checkbox" name="departments[]" value="{{ $dept->id }}" class="dept-checkbox" {{ is_array($selectedDepts) && in_array($dept->id, $selectedDepts) ? 'checked' : '' }} style="width: 16px; height: 16px;">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const existingSteps = @json(old('steps', $approval_schema->steps));
            
            if (existingSteps && existingSteps.length > 0) {
                existingSteps.forEach(step => {
                    addStep(step.level, step.user_id);
                });
            } else {
                addStep(1);
            }
